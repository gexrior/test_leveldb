...
//        std::cout << "my Tablebuilder::Add" << std::endl;
        Rep *r = rep_;
        assert(!r->closed);
        if (!ok()) return;
...
//            ParseInternalKey(r->last_key, &ikey);
//            std::cout << "key_size : " << ikey.user_key.ToString().size() << "   key:" << ikey.user_key.ToString() << std::endl;
...
//        if (estimated_block_size >= r->options.block_size) {
        if (r->num_entries != 0 && 0 == r->num_entries % 2) {
            Flush();
        }
    }
