...
namespace leveldb {
    ...
    static size_t TargetFileSize(const Options *options) {
        return options->max_file_size;  // 2MB
    }
...
// total compaction cover more than this many bytes.
    static int64_t ExpandedCompactionByteSizeLimit(const Options *options) {
        return 25 * TargetFileSize(options); // 50MB
    }
    ...
    static double MaxBytesForLevel(const Options *options, int level) {
        ...
        double result = 10. * 1048576.0;  // 10 * 2^20 为 10MB
        while (level > 1) {
            result *= 10;
            level--;
        }
    ...
    static uint64_t MaxFileSizeForLevel(const Options *options, int level) {
        ...
        return TargetFileSize(options);  // 2MB
    }
    ...
    // 给定 user_key, 返回 该键是否在给定的 FileMetaData 文件之前
    // 即 user_key < FileMetaData.smallest
    ...
    // 返回 [*smallest_user_key, *largest_user_key] 是否存在于 level[i] 的文件中
    bool SomeFileOverlapsRange(
        ...
        if (!disjoint_sorted_files) { // 即 level == 0, 需要查找所有的文件
    ...
    void Version::ForEachOverlapping(Slice user_key, Slice internal_key,
        ...
        if (!tmp.empty()) {
            ...
            for (uint32_t i = 0; i < tmp.size(); i++) {
                // Match(struct State state, int level, FileMetaData *f)
                if (!(*func)(arg, 0, tmp[i])) {
                    return;
                }
            }
...
//        std::cout << "Version::Get. key=" << k.ToString() << std::endl;
        ...
        for (int level = 0; level < config::kNumLevels; level++) {
            ...
            if (level == 0) {
                ...
                std::sort(tmp.begin(), tmp.end(), NewestFirst);
                files = &tmp[0];
                num_files = tmp.size();   // tmp.size() 大小和 reserve 无关，取决于 push_back
            } else {
                // Binary search to find earliest index whose largest key >= ikey.
                // 仅仅找到 largest key >= ikey，可能 smallest key > ikey
                // 找不到则返回 num_files
...
// Versions that contain full copies of the intermediate state.
    ...
    public:
        ...
        void Apply(VersionEdit *edit) {
            ...
            for (size_t i = 0; i < edit->new_files_.size(); i++) {
                ...
                f->allowed_seeks = static_cast<int>((f->file_size / 16384U));   // 2^14
    ...
    // 为什么要用锁？
    // DB::Impl 全局就只有一个 VersionSet 对象 versions_, 当然要加锁保护 versions_
    ...
    //
    // DB::Impl 中，传入的是 VersionSet->current
    ...
    void VersionSet::Finalize(Version *v) {
        ...
        for (int level = 0; level < config::kNumLevels - 1; level++) {
            // 对于 level == 0, score = 0层文件数 / 4
            // 对于 level >  0, score = 当前level字节数 / 该level最大字节数
            double score;
            if (level == 0) {
...
// REQUIRES: inputs is not empty
    ...
    // 此时 c 已经初始化, 即 c 的 level_、input_version_、inputs_[0] 等均非空, inputs_[1] 扔为空
    ...
    bool Compaction::IsTrivialMove() const {
        ...
        // 当前level 层文件数为1， level + 1 层文件数为 0
    ...
    bool Compaction::ShouldStopBefore(const Slice &internal_key) {
        ...
        // MaxGrandParentOverlapBytes = 20MB(10*options->max_file_size)
